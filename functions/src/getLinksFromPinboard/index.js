  ...
  try {
    ...
    async function loopOverUsers() {
      ...
      users.forEach(async (user) => {
        ...
        if (message !== null) {
          console.log(
            `Message for ${user} was ${message.u} . Going to send an email...`
          )
          await sendEmail(user, message).catch(console.error)
        } else {
          console.log('No message to send today')
        }
      })
    }
    ...
    async function upsertLink(user, linkData) {
      ...
      const queryOfBacklogForNewLink = await sharedLinksCollection
        ...
        .get()
      if (
        queryOfBacklogForNewLink.empty &&
        queryOfSharedLinksForNewLink.empty
      ) {
        console.log(
    ...
    console.error('There was an error in main funtion: ', err)
  }
}
